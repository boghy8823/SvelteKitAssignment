...
| `/[locale]/dashboard/items`   | SSR + streamed rows   | Node    | Owns mutable state and parses the 220-row dataset                                           |
...
the skeleton is the first paint and the rows arrive after. Post bodies do not qualify, because the
body is what LCP measures — streaming it would trade a real metric for an imaginary one.
